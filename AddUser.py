...
mydb = myclient["vote"]
mycol = mydb["data"]
def addVoter(name,password,fingerprint,id):
    for x in mycol.find({"id":id}):
        return("Voter Already Exists")
    x = mycol.insert_one({"name":name,"password":password,"fingerprint":fingerprint,"id":id,"vote":"0"})
    Face.addUserFace(id)
    return("Done")
...
def VerifyUser(id,password,fingerprint):
    for x in mycol.find({"id":id}):
